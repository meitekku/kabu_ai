...
## マーケット概況（必須）
レポートの冒頭に「マーケット概況」として、以下のSQLで前日のNYダウ終値を取得して記載すること:
node scripts/agent-db-query.cjs "SELECT date, close FROM price WHERE code='1' ORDER BY date DESC LIMIT 1"
例: 「前日ダウ: 38,500.00 (+0.5%)」のような形式で簡潔に。
