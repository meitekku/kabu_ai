...
export default function USAcceptAI() {
  ...
  const fetchData = async (): Promise<void> => {
    try {
      ...
      const response = await fetch('/api/admin/accept_ai_us', {
        method: 'POST',
        headers: {
  ...
  return (
    <main className="container mx-auto py-4">
      <h1 className="text-2xl font-bold mb-4">US株 AI記事管理</h1>
      <ApprovalList
        items={items}
        fetchData={fetchData}
        approveSiteNumber={80}
        apiEndpoint="/api/admin/accept_ai_us"
      />
    </main>
  );
}
