...
export default function Home() {
  ...
  const fetchData = async (): Promise<void> => {
    try {
      ...
      const response = await fetch('/api/admin/accept_spike', {
        method: 'POST',
        headers: {
  ...
  return (
    <main className="container mx-auto py-4">
      <h1 className="text-2xl font-bold mb-4">急増アラートリスト</h1>
      <ApprovalList items={items} fetchData={fetchData} approveSiteNumber={75} apiEndpoint="/api/admin/accept_spike" />
    </main>
  );
}
