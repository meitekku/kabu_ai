...
export function AgentChatSidebar({ selectedChatId, onSelectChat, onNewChat, refreshKey }: AgentChatSidebarProps) {
  ...
  const handleDelete = async (chatId: string, e: React.MouseEvent) => {
    ...
    try {
      await fetch(`/api/agent-chat/history?chatId=${chatId}`, { method: 'DELETE' });
      setChats(chats.filter((chat) => chat.id !== chatId));
      if (selectedChatId === chatId) {
        onNewChat();
      }
    } catch (error) {
      console.error('Failed to delete agent chat:', error);
    }
  };
